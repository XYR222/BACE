...
class TrajectoryCollector:
    ...
    def vanilla_multi_turn_loop(
        ...
        obs, infos = envs.reset(kwargs=reset_options if reset_options is not None else env_kwargs)
        ...
        if uid_batch is not None:
            ...
            if len(uid_batch) != batch_size:
                raise ValueError("uid_batch must match the rollout batch size")
        ...
        for _step in range(self.config.env.max_steps):
            ...
            if self._capture_bace_metadata():
                batch.non_tensor_batch['step_index'] = np.full(batch_size, _step, dtype=np.int32)
                batch.non_tensor_batch['admissible_actions'] = np.array(
                    obs.get('admissible_actions', [tuple() for _ in range(batch_size)]),
                    dtype=object,
                )
            ...
            if self._capture_bace_metadata():
                batch.non_tensor_batch['occurrence_id'] = np.array(
                    [f"{traj_uid[i]}:{_step}" for i in range(batch_size)], dtype=object
                )
                if task_batch_indices is None:
                    rollout_n = max(1, int(self.config.env.rollout.n))
                    task_batch_indices = [i // rollout_n for i in range(batch_size)]
                if len(task_batch_indices) != batch_size:
                    raise ValueError("task_batch_indices must match the rollout batch size")
                batch.non_tensor_batch['task_batch_index'] = np.asarray(
                    task_batch_indices, dtype=np.int32
                )
            ...
            if self._capture_bace_metadata():
                ...
                batch.non_tensor_batch['task_description'] = np.array(
                    [self_task for self_task in getattr(envs, 'tasks', [''] * batch_size)], dtype=object
                )
                batch.non_tensor_batch['done'] = np.asarray(dones, dtype=bool)
                batch.non_tensor_batch['remaining_horizon'] = np.full(
                    batch_size, self.config.env.max_steps - _step - 1, dtype=np.int32
                )
