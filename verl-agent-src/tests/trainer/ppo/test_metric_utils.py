...
class TestComputeDataMetrics(unittest.TestCase):
    ...
    def test_success_rate_is_reduced_over_unique_trajectories(self):
        ...
        batch.non_tensor_batch["traj_uid"] = np.asarray(["a", "a", "b", "b"], dtype=object)
        batch.non_tensor_batch["episode_rewards"] = np.asarray([10.0, 10.0, 0.0, 0.0])
        ...
        self.assertEqual(metrics["episode/success_rate_mean"], 0.5)
        self.assertEqual(metrics["episode/success_rate_min"], 0.0)
        self.assertEqual(metrics["episode/success_rate_max"], 1.0)

    def test_root_branch_and_mixed_success_use_terminal_rewards(self):
        batch = DataProto.from_single_dict(
            data={
                "token_level_scores": torch.zeros((4, 2)),
                "token_level_rewards": torch.zeros((4, 2)),
        ...
        batch.non_tensor_batch["traj_uid"] = np.asarray(
            ["root-a", "root-a", "branch-b", "branch-b"], dtype=object
        )
        ...
        batch.non_tensor_batch["episode_rewards"] = np.asarray(
            [10.0, 10.0, 0.0, 0.0], dtype=np.float32
        )
        batch.non_tensor_batch["episode_lengths"] = np.asarray([2.0] * 4)
        batch.non_tensor_batch["tool_callings"] = np.asarray([0.0] * 4)
        # Deliberately stale branch-origin metadata: reward remains canonical.
        batch.non_tensor_batch["success_rate"] = np.asarray([1.0, 1.0, 1.0, 0.0])
