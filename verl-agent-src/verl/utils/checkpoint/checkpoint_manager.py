...
def find_existing_component_checkpoints(local_path: str) -> list[str]:
    """Find same-component checkpoints up to the checkpoint being loaded.

    Checkpoint managers remember only paths saved by their current process.
    A segmented Slurm resume therefore starts with an empty retention list,
    even when an actor checkpoint from the previous process occupies disk.
    Rebuilding that list allows normal pre-save rotation to work across jobs.
    """
    component_path = Path(local_path).resolve()
    step_directory = component_path.parent
    checkpoint_root = step_directory.parent
    if not step_directory.name.startswith("global_step_"):
        return [str(component_path)]
    try:
        loaded_step = int(step_directory.name.removeprefix("global_step_"))
    except ValueError:
        return [str(component_path)]

    candidates: list[tuple[int, str]] = []
    for candidate_step_directory in checkpoint_root.glob("global_step_*"):
        try:
            step = int(
                candidate_step_directory.name.removeprefix("global_step_")
            )
        except ValueError:
            continue
        candidate = candidate_step_directory / component_path.name
        if step <= loaded_step and candidate.is_dir():
            candidates.append((step, str(candidate.resolve())))
