...
def _episode_success_groups(batch: DataProto) -> Dict[str, np.ndarray]:
    """Return canonical episode outcomes grouped by root/branch/mixed scope.

    ``episode_rewards`` is the source of truth.  Per-row success metadata can
    be copied from a root row when constructing a branch origin, so it must not
    be used for this aggregation.  A trajectory may have multiple rows and a
    branch may contain both ``branch_origin`` and ``branch_suffix`` rows.
    """
    rewards = np.asarray(batch.non_tensor_batch["episode_rewards"], dtype=np.float64)
    traj_uids = np.asarray(
        batch.non_tensor_batch.get("traj_uid", np.arange(len(rewards))), dtype=object
    )
    source_types = np.asarray(
        batch.non_tensor_batch.get("source_type", ["unknown"] * len(rewards)), dtype=object
    )
    episodes = {}
    ...
    for record in episodes.values():
        ...
        groups[scope].append(value)
        if scope != "mixed":
            groups["mixed"].append(value)
    return {scope: np.asarray(values, dtype=np.float64) for scope, values in groups.items()}


def _episode_success_metrics(groups: Dict[str, np.ndarray]) -> Dict[str, Any]:
    metrics = {}
    for scope, values in groups.items():
        if values.size == 0:
            continue
        prefix = f"episode/{scope}_success_rate"
        metrics[prefix] = float(np.mean(values))
    ...
    # Keep the historical key as an explicit alias for the mixed population.
    if "episode/mixed_success_rate" in metrics:
        metrics["episode/success_rate"] = metrics["episode/mixed_success_rate"]
        metrics["episode/success_rate_mean"] = metrics["episode/mixed_success_rate_mean"]
        metrics["episode/success_rate_min"] = metrics["episode/mixed_success_rate_min"]
        metrics["episode/success_rate_max"] = metrics["episode/mixed_success_rate_max"]
    return metrics
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # ``episode_rewards`` is canonical for root/branch/mixed success.  The
    # legacy success_rate metadata is retained for compatibility below, but is
    # not trusted for the core outcome metrics.
    success_rate_metrics = _episode_success_metrics(_episode_success_groups(batch))
    canonical_success_keys = {
        "success_rate", "root_success_rate", "branch_success_rate", "mixed_success_rate"
    }
    for key, values in batch.non_tensor_batch.items():
        if "success_rate" not in key or key in canonical_success_keys:
            continue
        values = np.asarray(values, dtype=np.float64)
        episode_values = values[unique_idx] if len(values) == len(batch) else values
        if episode_values.size == 0:
            continue
        metric_key = f"episode/{key}"
        mean_value = float(np.mean(episode_values))
        success_rate_metrics[metric_key] = mean_value
        success_rate_metrics[f"{metric_key}_mean"] = mean_value
        success_rate_metrics[f"{metric_key}_min"] = float(np.min(episode_values))
        success_rate_metrics[f"{metric_key}_max"] = float(np.max(episode_values))
