...
class RayPPOTrainer:
    ...
    def _save_checkpoint(self):
        ...
        if hasattr(self.traj_collector, "state_dict"):
            collector_state_path = os.path.join(
                local_global_step_folder, "bace_collector_state.json"
            )
            collector_state = {
                "global_step": self.global_steps,
                "collector": self.traj_collector.state_dict(),
            }
            temporary_path = f"{collector_state_path}.tmp-{uuid.uuid4().hex}"
            with open(temporary_path, "w", encoding="utf-8") as stream:
                json.dump(collector_state, stream, indent=2, sort_keys=True)
                stream.flush()
        ...
        with open(temporary_tracker, "w") as f:
    ...
    def _load_checkpoint(self):
        ...
        collector_state_path = os.path.join(
            global_step_folder, "bace_collector_state.json"
        )
        requires_bace_state = getattr(
            self.traj_collector, "requires_checkpoint_state", False
        )

        # Validate every controller-side component before mutating the actor or
        # dataloader. Dynamic BACE must never resume an actor while silently
        # resetting either the sampler cursor or lagged family history.
        if os.path.exists(dataloader_local_path):
            dataloader_state_dict = torch.load(dataloader_local_path, weights_only=False)
        else:
            dataloader_state_dict = None
            if requires_bace_state:
                raise FileNotFoundError(
                    f"Dynamic BACE checkpoint is missing dataloader state: {dataloader_local_path}"
                )

        collector_state = None
        if hasattr(self.traj_collector, "load_state_dict"):
            if not os.path.exists(collector_state_path):
                if requires_bace_state:
                    raise FileNotFoundError(
                        "Dynamic BACE checkpoint is missing collector state: "
                        f"{collector_state_path}"
                    )
            else:
                with open(collector_state_path, encoding="utf-8") as stream:
                    saved_collector_state = json.load(stream)
                if int(saved_collector_state.get("global_step", -1)) != self.global_steps:
                    raise ValueError(
                        "BACE collector checkpoint global_step does not match actor checkpoint"
                    )
                collector_state = saved_collector_state.get("collector")
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        ################ agent-environment loop ###############
                        if hasattr(self.traj_collector, "set_step"):
                            self.traj_collector.set_step(self.global_steps)
                        ...
                        if hasattr(self.traj_collector, "last_bace_metrics"):
                            metrics.update(
                                {
                                    f"bace/{key}": value
                                    for key, value in self.traj_collector.last_bace_metrics.items()
                                }
                            )
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            gigpo_compute_mean_std_cross_steps=self.config.algorithm.gigpo.get(
                    ...
                    # validate
                    validate_on_last_step = bool(
                        self.config.trainer.get("validate_on_last_step", True)
                    )
                    should_validate = (
                        self.config.trainer.test_freq > 0
                        and (
                            self.global_steps % self.config.trainer.test_freq == 0
                            or (is_last_step and validate_on_last_step)
                        )
                    )
