...
class TaskRunner:
    def run(self, config):
        ...
        bace_config = config.algorithm.get("bace", None)
        if bace_config and bace_config.get("enabled", False):
            if str(config.algorithm.adv_estimator) != "bace_gigpo":
                raise ValueError(
                    "algorithm.bace.enabled=true requires "
                    "algorithm.adv_estimator=bace_gigpo"
                )
            topology = str(bace_config.get("topology", "fixed"))
            root_count = (
                int(bace_config.total_leaf_budget)
                if topology == "dynamic"
                else int(bace_config.fixed_root_count)
            )
            if int(config.env.rollout.n) != root_count:
                raise ValueError(
                    "env.rollout.n must equal the dynamic total_leaf_budget or fixed_root_count"
                )
            ...
            traj_collector = BaceTrajectoryCollector(
                config=config,
                tokenizer=tokenizer,
                processor=processor,
