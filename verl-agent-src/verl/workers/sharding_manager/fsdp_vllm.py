...
def _get_vllm_tp_process_group():
    """Return the torch process group for vLLM tensor parallel collectives."""
    if hasattr(vllm_ps, "get_tensor_model_parallel_group"):
        group = vllm_ps.get_tensor_model_parallel_group()
    elif hasattr(vllm_ps, "get_tp_group"):
        group = vllm_ps.get_tp_group()
    else:
        raise AttributeError("vLLM parallel_state has no tensor-parallel group accessor")
...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        ...
        group = _get_vllm_tp_process_group()
