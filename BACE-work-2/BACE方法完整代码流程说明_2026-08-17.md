...
`ExactBatchErvEngine.global_allocation()` 使用 quota-aware exact DP 决定每个
...
DP 只保留 $q\in[0,Q_{task}]$ 的可达状态，复杂度为
$O(AQ_{task}L_{\max})$。每个状态记录最优值、最优前缀路径数和压缩的并列
predecessors，不物化完整 global tie set。若有多个最优 allocation，使用
SHA256(global seed、policy update id、task id、上下文) 派生稳定随机种子，
按 predecessor path count 加权回溯，从而对完整最优 allocations 做严格均匀选择。
随后依次：

1. 通过 count-weighted DP backtracking 均匀选一个全局最优 allocation；
2. 对每个已分配 anchor，在其局部最优动作 multiset ties 中均匀选一个；
3. 对 multiset 中每个动作，在该 action 的自然 origins 中均匀选一个。

旧 Cartesian solver 保留为小规模测试 oracle，并带 $10^6$ 状态 hard guard，
生产路径不再调用。当前 exact 模式会把整批
